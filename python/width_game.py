...
class WidthGame:
    ...
    async def handle_tick(self, browser_ids: Dict):
        if self.state != WidthGameState.OVER:
            if self.time_left == 1:
                self.state = WidthGameState.OVER
                await self.resolve(browser_ids)
            else:
                self.time_left -= 1
    ...
    async def resolve(self, browser_ids: Dict):
        if self.state != WidthGameState.OVER:
        ...
        elif self.best_player not in browser_ids:
            raise Exception("WidthGame.resolve called but best_player not in browser_ids")

        payload = {
            "winner_browser_id": browser_ids[self.best_player],
